...
from django_social import utils
...
def public_profile(request, username) -> HttpResponse:
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated and user != account:
            ...
            if friends.filter(pk=user.id):
                ...
                if (
                    get_friend_request_or_false(sender=account, receiver=user)
                    is not False
                ):
                ...
                # CASE 2: request you sent to them
                    ...
                    get_friend_request_or_false(sender=user, receiver=account)
                    is not False
                ):
